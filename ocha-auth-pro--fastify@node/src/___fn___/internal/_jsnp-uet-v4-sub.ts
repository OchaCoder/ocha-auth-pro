 ...
 *
 *     { uet: string; id: number; nextEmail: string }
 *
 ...
 * @returns { uet: string; id: number; nextEmail: string }
 * @throws {ErrorDefensiveGuardBreach}
 */
export const jsnpUetV4Sub = (sub: string): { uet: string; id: number; nextEmail: string } => {
  const parsedSub = JSON.parse(sub) as unknown
  const context = {
    file: `_jsnp-uet-v4-sub.ts`,
    fn: `jsnpUetV4Sub`,
    specialJson: sub,
    debug: parsedSub,
  }
  ...
  if (!("uet" in parsedSub) || !("id" in parsedSub) || !("nextEmail" in parsedSub)) {
    const code = `ERR_V4_SUB_MISSING_PROPERTY`
    const message = `The 'sub' should have 'uet', 'id' and 'nextEmail'. Likely a bug during token generation.`
    throw new ErrorDefensiveGuardBreach(code, message, context)
  }

  if (typeof parsedSub.uet !== "string" || typeof parsedSub.id !== "number" || !TbV.util.isEmail.Check(parsedSub.nextEmail)) {
  ...
  const validatedRptObj = { uet: parsedSub.uet, id: parsedSub.id, nextEmail: parsedSub.nextEmail }
