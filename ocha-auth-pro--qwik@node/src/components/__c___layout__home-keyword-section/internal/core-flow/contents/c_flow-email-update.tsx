...
export const FlowEmailUpdate = component$(({ code }: { code: ModalKeywordCode }) => {
  const classes = `
        flex-column justify-start
        p-10
        ${code === modalKeywordCode.coreFlow.EMAIL_UPDATE ? `flex visible opacity-1` : `none hidden opacity-0`}
        `
  return (
    <div class={classes} style={{ paddingTop: "45px", paddingBottom: "45px" }}>
      <h1 class={`font-size-18 color-theme-sub p-tb-5`}>Email Update</h1>
      <p class="font-size-13 color-gray p-b-5">A simplified view of email update.</p>
      ...
      <section class={`p-tb-5`}>
        ...
        <div class={`relative`}>
          ...
          <div class={`flex flex-column p-tb-10 gap-10 items-center`} style={{ flexWrap: "wrap" }}>
            ...
            <DiagramArrow />
            <DiagramStepB step={`Email Exists Check`} />
            <DiagramArrow />
