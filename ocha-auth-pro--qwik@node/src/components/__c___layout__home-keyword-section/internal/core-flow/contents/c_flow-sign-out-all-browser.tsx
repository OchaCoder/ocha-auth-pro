...
export const FlowSignOutAllBrowser = component$(({ code }: { code: ModalKeywordCode }) => {
  const classes = `
        flex-column justify-start
        p-10
        ${code === modalKeywordCode.coreFlow.SIGN_OUT_ALL_BROWSER ? `flex visible opacity-1` : `none hidden opacity-0`}
        `
  return (
    <div class={classes} style={{ paddingTop: "45px", paddingBottom: "45px" }}>
      <h1 class={`font-size-18 color-theme-sub p-tb-5`}>Sign Out (All Browsers)</h1>
      ...
      <section class={`p-tb-5`}>
        ...
        <div class={`relative`}>
          ...
          <div class={`flex flex-column p-tb-10 gap-10 items-center`} style={{ flexWrap: "wrap" }}>
            ...
            <DiagramArrow />
            <DiagramStepB step={`Get All Browser IDs Using User ID`} />
            <DiagramStepB step={`Remove All Browser IDs`} />
            <DiagramArrow />
